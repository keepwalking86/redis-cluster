...
redis_cmd='redis-cli -h 192.168.1.240 -p 30001'
...
# filter keys hava string "redis" and sizes >1000000B (~1M)
for k in `$redis_cmd keys "*"`; do key_size_bytes=`$redis_cmd debug object $k | perl -wpe 's/^.+serializedlength:([\d]+).+$/$1/g'`;  if [[ $k == redis* ]] && [[ $key_size_bytes -gt 1024 ]]; then size_key_list="$size_key_list$key_size_bytes $k\n"; fi done
